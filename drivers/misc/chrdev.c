			...
			char __user *buf, size_t count, loff_t *ppos)
{
	...
	dev_info(chrdev->dev, "shoud read %d bytes (*ppos=%lld)\n",
				count, *ppos);
	...
	*ppos += count;
	dev_info(chrdev->dev, "return %d bytes (*ppos=%lld)\n", count, *ppos);
			...
			const char __user *buf, size_t count, loff_t *ppos)
{
	...
	dev_info(chrdev->dev, "shoud write %d bytes (*ppos=%lld)\n",
				count, *ppos);
	...
	*ppos += count;
	dev_info(chrdev->dev, "got %d bytes (*ppos=%lld)\n", count, *ppos);
...
static int __init chrdev_init(void)
{
	...
	if (!chrdev_class) {
		pr_err("chrdev: failed to allocate a class\n");
		return -ENOMEM;
	}
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Byron Lathi (Rodolfo Giometti)");
